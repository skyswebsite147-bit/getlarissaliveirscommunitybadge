...
A fan community landing page for artist Larissa Liveira. Fans browse badge membership tiers, select a payment method (which triggers a pre-filled mailto to management), and find customer support contact info. Built with TanStack Start and deployed on Netlify. No database or authentication is used.

### Key Files Changed from Scaffold
- `src/routes/index.tsx` — Complete rewrite; the entire site lives here
- `src/styles.css` — Tailwind + concert animation keyframes (spotlights, shimmer, bg pulse)
- `src/routes/__root.tsx` — Updated title/meta for the Larissa Liveira brand
- `public/larissa-bg.jpeg` — Artist photo used as the animated concert background
